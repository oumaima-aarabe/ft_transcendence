...
import { Flame, Waves, Trophy, Star } from 'lucide-react';
import { 
  GameDifficulty, 
  GameTheme,
  KeyStates
...
const PongGame: React.FC<PongGameProps> = ({
  ...
  const [uiState, setUiState] = useState({
    gameStatus: 'menu' as 'menu' | 'playing' | 'paused' | 'matchOver' | 'gameOver',
  ...
  // Toggle pause state
  const togglePause = () => {
    const newStatus = gameStateRef.current.gameStatus === 'playing' ? 'paused' : 
                     gameStateRef.current.gameStatus === 'paused' ? 'playing' : 
  ...
  const updateGameState = () => {
    ...
    if (keys['s'] && leftPaddle.y + leftPaddle.height < BASE_HEIGHT) {
      leftPaddle.y = Math.min(BASE_HEIGHT - leftPaddle.height, leftPaddle.y + leftPaddle.speed);
    }
    ...
    if (keys['ArrowDown'] && rightPaddle.y + rightPaddle.height < BASE_HEIGHT) {
      rightPaddle.y = Math.min(BASE_HEIGHT - rightPaddle.height, rightPaddle.y + rightPaddle.speed);
    }
  ...
  const resetBall = (ball: EnhancedGameState['ball'], direction: number, initialSpeed: number) => {
    ball.x = BASE_WIDTH / 2;
  ...
  const renderGame = (ctx: CanvasRenderingContext2D, themeProps: any) => {
    const { ball, leftPaddle, rightPaddle, gameStatus, matchWins, winner } = gameStateRef.current;
    ...
    ctx.textAlign = 'right';
    ctx.fillText(`Points: ${rightPaddle.score}`, BASE_WIDTH - 20, BASE_HEIGHT - 20);
    ctx.restore();
      ...
      if (theme === 'fire') {
        streaks.push(
          ...
          >
            <Flame 
              size={24} 
              className={`${isActive ? 'text-orange-500 drop-shadow-[0_0_5px_rgba(208,95,59,0.8)]' : 'text-gray-500'}`}
            />
          </div>
        );
      } else {
        streaks.push(
          ...
          >
            <Waves 
              size={24} 
              className={`${isActive ? 'text-teal-400 drop-shadow-[0_0_5px_rgba(64,207,183,0.8)]' : 'text-gray-500'}`}
            />
          </div>
        );
      }
    ...
    <div className="w-full flex flex-col items-center justify-center">
      ...
      <div className="mb-6 flex flex-wrap gap-4 justify-center">
        ...
        <Button 
          ...
          disabled={uiState.gameStatus === 'menu' || uiState.gameStatus === 'matchOver' || uiState.gameStatus === 'gameOver'}
        >
      ...
      >
        ...
        <div 
          className={`mb-3 p-4 rounded-xl flex items-center justify-between ${
            theme === 'fire' ? 'bg-black/80 border-[#D05F3B]' : 'bg-black/80 border-[#40CFB7]'
          } border-2`}
          style={{
            boxShadow: theme === 'fire' 
        ...
        >
                ...
                <div className="flex flex-col items-start">
                  ...
                  {uiState.matchWins.player1 > 0 && (
                    <span className={`text-xs ${
                      ...
                      {uiState.matchWins.player1} {uiState.matchWins.player1 === 1 ? 'match' : 'matches'} won
                    </span>
                  )}
                </div>
              </div>
          ...
          <div className="flex flex-col items-center">
            <div className={`text-xl font-bold mb-1 ${
              theme === 'fire' && theme === 'water' ? 'bg-gradient-to-r from-[#D05F3B] to-[#40CFB7] text-transparent bg-clip-text' : 
              theme === 'fire' ? 'text-[#D05F3B]' : 'text-[#40CFB7]'
            }`}>
            ...
            <div className="mt-1 text-xs text-gray-400">First to {POINTS_TO_WIN_MATCH} points</div>
          </div>
        ...
        <div className="mt-4 flex flex-col items-center">
          <div className={`flex items-center justify-center gap-2 mb-2 ${
            ...
            <Trophy size={16} />
            <span className="font-medium">First to win {MATCHES_TO_WIN_GAME} matches wins the game!</span>
          </div>
